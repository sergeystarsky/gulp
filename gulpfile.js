...
gulp.task('build', ['clean', 'img', 'sass', 'scripts'], function(){
	...
	var buildCss = gulp.src([   //Создаем переменную build
		'app/css/main.css',  //указываем файлы, которые импортируем в папку продакшена dist/css - в данном случае
		'app/css/libs.min.css', //указываем файлы, которые импортируем в папку продакшена dist/css - в данном случае
	])
	.pipe(plumber())
	.pipe(gulp.dest('dist/css'));  // переносим наши файлы в папку dist/css

	var buildFonts = gulp.src('app/fonts/**/*')//новая перменная, берем все файлы из директории
		.pipe(gulp.dest ('dist/fonts'));  // переносим в папку продакшена dist
